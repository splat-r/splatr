...
def coarse_training_with_sdf_regularization(args, gs_model):
    ...
    save_milestones = [5000, 7000, 9000, 12_000, 15_000]
    ...
    for batch in range(9_999_999):
        ...
        for i in range(0, train_num_images, train_num_images_per_batch):
            ...
            if not no_rendering:
                ...
                if not args.walkthrough:
                    save_path_act = "output/sim_walk"
                    if not os.path.exists(save_path_act):
                        os.makedirs(save_path_act)
                    image_name_act = nerfmodel.get_image_name(camera_indices)
                    torchvision.utils.save_image(outputs['image'].permute(2, 0, 1),
                                                 os.path.join(save_path_act, '{}.png'.format(image_name_act)))
                ...
                if not args.include_feature:
                    if args.walkthrough:
                        ...
                        writer.add_scalar('gs_loss', loss, iteration)
                        ...
                        if not args.var_loss:
                            ...
                            if iteration < args.start_var_reg_from:
                                ...
                                if gs_model is not None:
                                    ...
                                    walk_rgb = rendered_image_walk['image'].detach().view(-1,
                                                                     sugar.image_height,
                                                                     sugar.image_width,
                                                                     3)
                                    walk_rgb = walk_rgb.transpose(-1, -2).transpose(-2, -3)
                                    loss_var = loss_fn(pred_rgb, walk_rgb)
                                    loss = loss + 0.1*loss_var
                                    writer.add_scalar('var_loss', loss_var, iteration)

                                    save_path_rend = "output/sim_unsh"
                                    if not os.path.exists(save_path_rend):
                                        os.makedirs(save_path_rend)
                                    image_name_rend = nerfmodel.get_image_name(camera_indices)
                                    torchvision.utils.save_image(outputs['image'].permute(2, 0, 1),
                                                                 os.path.join(save_path_rend,
                                                                              '{}.png'.format(image_name_rend)))
                                else:
                                    loss = loss_fn(pred_rgb, gt_rgb)
                                    writer.add_scalar('gs_loss', loss, iteration)

                    if not args.surface_align:
                        enforce_entropy_regularization = True
                    ...
                    loss = l1_loss(language_feature*language_feature_mask, gt_language_feature*language_feature_mask)              
            else:
                loss = 0.
