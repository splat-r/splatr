...
import navigation.utils.depth_utils as du
import navigation.utils.rotation_utils as ru
import copy
...
class Mapper():
    def __init__(self, C, sc, origin, map_size, resolution, max_depth=164, z_bins=[0.05,3], max_obj=50,loc_on_map_selem = skimage.morphology.disk(2), bounds=None):
        ...
        self.rearrage_map = np.zeros((map_sz, map_sz), dtype=np.float32)
    ...
    def add_change_to_map(self, position, rotation, elevation, depth, masks):
        d = depth * 1.
        d[d > self.max_depth] = 0
        d[d < 0.02] = np.NaN
        d = d / self.sc
        ...
        for mask in masks:
            ...
            counts, is_valids, _ = du.bin_points(xyz3, self.map.shape[0], [], self.resolution)
            counts[0, 0] = 0
            cur_count_map = counts[:, :, 0]
            self.rearrage_map += cur_count_map
    ...
    def get_goal_position_on_map(self, pos_world):
        pos_map = (pos_world - self.origin_xz)/self.resolution + self.origin_map
        pos_map = pos_map.astype(np.int32)
        return pos_map
    ...
    def get_position_on_map_from_aithor_position(self, position_origin):
        xz = np.array([position_origin['x'], position_origin['z']], np.float32)
        return self.convert_xz_to_map_pos(xz)

    def get_position_map_from_aithor_pcd(self, pcd):
        print("pcd : ", pcd)
        map_position = pcd - self.origin_xz + self.origin_map * self.resolution
        print("map pos : ", map_position)
        print("ori : ", self.origin_xz)
        print("ori map : ", self.origin_map)
        print("res : ", self.resolution)
        map_position_ = np.round(map_position / self.resolution).astype(np.int32)
        print("map pos _ : ", map_position_)
    ...
    def add_obstacle_in_front_of_agent(self, selem, size_obstacle=10, pad_width=7):
        ...
        # print("Dilating obstacle map... ")
        # self.added_obstacles = skimage.morphology.binary_erosion(self.added_obstacles)

    def dilate_obstacles(self, iter = 3):
        for i in range(iter):
            self.added_obstacles = skimage.morphology.binary_erosion(self.added_obstacles)

    def add_obstacles_map_from_pcd(self, map_pos):
        map_pos = map_pos.astype(np.int32)
        map_pos_ = np.zeros_like(map_pos)
        map_pos_[:, 0] = copy.deepcopy(map_pos[:, 1])
        map_pos_[:, 1] = copy.deepcopy(map_pos[:, 0])
        self.added_obstacles[map_pos_] = False
    ...
    def dilate_obstacles_around_agent(self):
        '''
        ...
        y_end = int(y + radius)
        obstacle_ = self.added_obstacles[y_start:y_end, x_start:x_end]
        obstacle_ = skimage.morphology.binary_erosion(obstacle_)
        self.added_obstacles[y_start:y_end, x_start:x_end] = obstacle_

    def get_obs_map_(self):
