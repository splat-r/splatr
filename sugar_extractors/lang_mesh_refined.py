...
def extract_lang_mesh_from_lang_sugar(args):
    ...
    if args.mesh_output_dir is None:
        if len(args.scene_path.split("/")[-1]) > 0:
            args.mesh_output_dir = os.path.join("./output/refined_new_mesh", args.scene_path.split("/")[-1])
        else:
            args.mesh_output_dir = os.path.join("./output/refined_new_mesh", args.scene_path.split("/")[-2])
    ...
    if use_vanilla_3dgs:
        ...
        sugar = SuGaR(
            ...
            surface_mesh_to_bind=o3d_mesh,  # Open3D mesh
        )
        with torch.no_grad():
    ...
    if not use_marching_cubes:
            ...
            with torch.no_grad():
                ...
                for cam_idx in range(len(nerfmodel.training_cameras)):
                    if cam_idx % 30 == 0:
                        ...
                        for surface_level in surface_levels:
                            CONSOLE.print(
                                f"Current point cloud for level {surface_level} has {len(surface_levels_outputs[surface_level]['points'])} points.")

                    point_depth = cameras_to_use.p3d_cameras[cam_idx].get_world_to_view_transform().transform_points(
                        sugar.points)[..., 2:].expand(-1, 3)
                    ...
                    rgb = rgb.clamp(min=0., max=1.).contiguous()
                    import cv2
                    cv2.imwrite("output/image.png", (rgb.cpu().numpy() * 255).astype(np.uint8))
                    ...
                    with torch.no_grad():
                        if use_fast_method:
                            frame_surface_level_outputs = sugar.compute_level_surface_points_from_camera_fast(
                                ...
                                n_surface_points=2 * n_pts_per_frame,
                                # TODO: 2*n_pts_per_frame is safe to avoid empty pixels
                        ...
                        for surface_level in surface_levels:
                            ...
                            img_surface_view_directions = torch.nn.functional.normalize(
                                cameras_to_use.p3d_cameras[cam_idx].get_camera_center() - img_surface_points)
                            ...
                            surface_levels_outputs[surface_level]['points'] = torch.cat(
                                [surface_levels_outputs[surface_level]['points'], img_surface_points[idx]], dim=0)
                            surface_levels_outputs[surface_level]['colors'] = torch.cat(
                                [surface_levels_outputs[surface_level]['colors'], img_surface_colors[idx]], dim=0)
                            surface_levels_outputs[surface_level]['view_directions'] = torch.cat(
                                [surface_levels_outputs[surface_level]['view_directions'],
                                 img_surface_view_directions[idx]], dim=0)
                            surface_levels_outputs[surface_level]['pix_to_gaussians'] = torch.cat(
                                [surface_levels_outputs[surface_level]['pix_to_gaussians'],
                                 img_surface_pix_to_gaussians[idx]], dim=0)
                            surface_levels_outputs[surface_level]['normals'] = torch.cat(
                                [surface_levels_outputs[surface_level]['normals'], img_surface_normals[idx]], dim=0)

            # -----Processing surface levels-----
            for surface_level in surface_levels:
                CONSOLE.print("\n========== Processing surface level", surface_level, "==========")
                CONSOLE.print(
                    f"Final point cloud for level {surface_level} has {len(surface_levels_outputs[surface_level]['points'])} points.")
                ...
                if use_custom_bbox:
                    ...
                    fg_bbox_min_tensor = - fg_bbox_factor * sugar.get_cameras_spatial_extent() * torch.ones(1, 3,
                                                                                                            device=sugar.device)
                    fg_bbox_max_tensor = fg_bbox_factor * sugar.get_cameras_spatial_extent() * torch.ones(1, 3,
                                                                                                          device=sugar.device)

                if center_bbox:
                    _cameras_spatial_extent, _camera_average_xyz = sugar.get_cameras_spatial_extent(
                        return_average_xyz=True)
                    with torch.no_grad():
                ...
                fg_mask = (surface_points[points_idx] > fg_bbox_min_tensor).all(dim=-1) * (
                            surface_points[points_idx] < fg_bbox_max_tensor).all(dim=-1)
                if center_bbox:
                    bg_mask = ((surface_points[points_idx] - _camera_average_xyz).abs().max(dim=-1)[0]
                               < bg_bbox_factor * _cameras_spatial_extent) * ~fg_mask
                else:
                    bg_mask = (surface_points[points_idx].abs().max(dim=-1)[
                                   0] < bg_bbox_factor * sugar.get_cameras_spatial_extent()) * ~fg_mask
                ...
                if fg_points.shape[0] > 0:
                    ...
                    o3d_fg_mesh, o3d_fg_densities = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(
                        fg_pcd,
                        depth=poisson_depth)  # , width=0, scale=1.1, linear_fit=False)  # depth=10 should be the default value? 11 is good to (but it starts to make a big number of triangles)
                ...
                for decimation_target in decimation_targets:
                    ...
                    if (decimated_o3d_fg_mesh is not None) and (decimated_o3d_bg_mesh is not None):
                        ...
                        raise ValueError(
                            "Both foreground and background meshes are empty. Please provide a valid bounding box for the scene.")

                    if use_vanilla_3dgs:
                        sugar_mesh_path = 'sugarmesh_vanilla3dgs_levelZZ_decimAA.ply'
                    else:
                        sugar_mesh_path = 'sugarmesh_' + sugar_checkpoint_path.split('/')[-2].replace('sugarcoarse_',
                                                                                                      '') + '_levelZZ_decimAA.ply'
                    ...
                    o3d.io.write_triangle_mesh(sugar_mesh_path, decimated_o3d_mesh, write_triangle_uvs=True,
        ...
        with torch.no_grad():
            ...
            verts = -sugar.get_cameras_spatial_extent() + (
                        torch.tensor(vertices) / resolution) * 2 * sugar.get_cameras_spatial_extent()
        ...
        with torch.no_grad():
            ...
            densities[(pts > -sugar.get_cameras_spatial_extent()).all(dim=-1) * (
                        pts < sugar.get_cameras_spatial_extent()).all(dim=-1)] = 0.
            densities = densities.reshape(resolution, resolution, resolution)

            density_th = surface_levels[0]  # 1.
            CONSOLE.print(f"Computing mesh for surface level {density_th}...")
            bg_vertices, bg_triangles = mcubes.marching_cubes(densities.cpu().numpy(), density_th)
            bg_verts = - 4 * sugar.get_cameras_spatial_extent() + (
                        torch.tensor(bg_vertices) / resolution) * 2 * 4 * sugar.get_cameras_spatial_extent()
        ...
        if use_vanilla_3dgs:
            ...
            sugar_mesh_path = 'sugarmesh_' + sugar_checkpoint_path.split('/')[-2].replace('sugarcoarse_',
                                                                                          '') + 'marchingcubes_levelZZ_decimAA.ply'
        sugar_mesh_path = sugar_mesh_path.replace(
            'ZZ', str(surface_level).replace('.', '')
        ).replace(
            'AA', str(decimation_target).replace('.', '')
