...
class GaussianSplattingWrapper:
    ...
    def get_image_name(self, camera_indices:int):
        return self.cam_list[camera_indices].image_name

    def get_gt_lang_ft(self, camera_indices, language_feature_dir, feature_level=0, to_cuda=False, langsplat=False):
        """
        Returns the ground truth language features corresponding to the training camera at the current index.
        Args:
            camera_indices (int): Index of the camera in the set of cameras.
            language_feature_dir: Path to the dir that contains language features.
            feature_level: {0, 1, 2, 3} Corresponds to different levels of abstraction with CLIP
            to_cuda (bool, optional): If True, moves the language feature to GPU. Defaults to False.

        Returns:
            Tensor: The ground truth language feature vector.
        """
        def delineate_masks(masks):
            mask_max_idx = torch.max(masks)
            masks_all = []
            if mask_max_idx > -1:
                for i in range(mask_max_idx+1):
                    masks_all.append(masks == i)
            else:
                masks_all.append(torch.ones_like(masks, dtype=torch.bool))
            return masks_all

        if langsplat:
            image_name = self.cam_list[camera_indices].image_name
            ...
            if feature_level == 0: # default
                ...
                seg_all_0 = seg_all[0:1].reshape(self.image_height, self.image_width)
                # mask_all_ret = delineate_masks(seg_all_0)
                mask_all_ret = copy.deepcopy(seg_all_0)
                ...
                seg_all_1 = seg_all[1:2].reshape(self.image_height, self.image_width)
                # mask_all_ret = delineate_masks(seg_all_1)
                mask_all_ret = copy.deepcopy(seg_all_1)
                ...
                seg_all_2 = seg_all[2:3].reshape(self.image_height, self.image_width)
                # mask_all_ret = delineate_masks(seg_all_2)
                mask_all_ret = copy.deepcopy(seg_all_2)
                ...
                seg_all_3 = seg_all[3:4].reshape(self.image_height, self.image_width)
                # mask_all_ret = delineate_masks(seg_all_3)
                mask_all_ret = copy.deepcopy(seg_all_3)
            else:
            ...
            seg_all_0 = seg_all[0:1].reshape(self.image_height, self.image_width)
            mask_all_ret_0 = copy.deepcopy(seg_all_0)

            seg_all_1 = seg_all[1:2].reshape(self.image_height, self.image_width)
            mask_all_ret_1 = copy.deepcopy(seg_all_1)

            seg_all_2 = seg_all[2:3].reshape(self.image_height, self.image_width)
            mask_all_ret_2 = copy.deepcopy(seg_all_2)

            seg_all_3 = seg_all[3:4].reshape(self.image_height, self.image_width)
            mask_all_ret_3 = copy.deepcopy(seg_all_3)
            ...
            if to_cuda:
                ...
                mask = mask.cuda()
            return point_feature, mask, mask_all_ret, image_name, mask_all_ret_0, mask_all_ret_1, mask_all_ret_2, mask_all_ret_3
        else:
            image_name = self.cam_list[camera_indices].image_name
            sim_file_name = os.path.join(language_feature_dir, "{}.pickle".format(image_name))
            sim_mask_file_name = os.path.join(language_feature_dir, "{}_mask.pickle".format(image_name))

            with open(sim_file_name, "rb") as lf:
                sim_ft = pickle.load(lf)
            with open(sim_mask_file_name, "rb") as lf:
                sim_mask = pickle.load(lf)

            sim_ft = torch.from_numpy(sim_ft)
            sim_ft = sim_ft.reshape(sim_ft.shape[0], sim_ft.shape[1], 1).permute(2, 0, 1)
            sim_mask = torch.from_numpy(sim_mask)
            if to_cuda:
                sim_ft = sim_ft.cuda()
                sim_mask = sim_mask.cuda()
            return sim_ft, sim_mask
