...
class SuGaR(nn.Module):
    ...
    def __init__(
        ...
        include_feature = False, # language features
        lang_ft_dim = 1,
        sim_vec = None,
        coarse = False,
        ...
        *args, **kwargs) -> None:
        ...
        self.learnable_shifts = learnable_positions
        ...
        if surface_mesh_to_bind is not None:
            ...
            print(torch.tensor(np.array(surface_mesh_to_bind.triangles)).shape)

            # verts_normals = torch.tensor(np.array(surface_mesh_to_bind.vertex_normals)).float().to(nerfmodel.device)

            self._vertex_colors = (torch.ones_like(torch.tensor(np.array(surface_mesh_to_bind.vertices)))*0.5).float().to(nerfmodel.device)
            print(torch.max(torch.tensor(np.array(surface_mesh_to_bind.triangles)).reshape(-1, )))
        ...
        if sim_vec is not None:
            self._sim_vec = nn.Parameter(sim_vec, requires_grad=True).to(nerfmodel.device)
        ...
        self.surface_mesh_to_bind = surface_mesh_to_bind
        self.beta_mode = beta_mode
    ...
    def reset_grads(self):
        ...
        if self.surface_mesh_to_bind is not None:
            self._surface_mesh_faces.detach()
            self.surface_mesh_thickness.detach()
        self._points.detach()
        self.all_densities.detach()
        self._scales.detach()
        self._quaternions.detach()
        self._sh_coordinates_dc.detach()
        self._sh_coordinates_rest.detach()
        # if self.include_feature:
        #     self._language_feature.detach()

    def set_grads_lang(self):
        print("setting grads")
        if self.surface_mesh_to_bind is not None:
            self._surface_mesh_faces.requires_grad_(False)
        ...
        if self.beta_mode == "learnable":
            self._log_beta.requires_grad_(False)

        print("setting language grad")
        self._language_feature = nn.Parameter(self._language_feature.requires_grad_(True))

    def reset_grads_lang(self):
        self._language_feature.requires_grad_(False)

    def init_refined_sim_vec(self, language_ft, text_ft_vec, language_thresh):
        cosine_sim = self.get_cos_sim(language_ft, text_ft_vec)
        cosine_sim_ = list(cosine_sim.detach().cpu().numpy())
        #self.sim_vec = nn.Parameter(torch.tensor(cosine_sim_, device=self.nerfmodel.device), requires_grad=True).to(self.nerfmodel.device)
        self.sim_vec = nn.Parameter(torch.tensor(cosine_sim, device=self.nerfmodel.device), requires_grad=True).to(
            self.nerfmodel.device)
        self._language_feature.requires_grad_(False)
        return cosine_sim

    def init_lang_ft(self, lang_ft_dim):
        self.reset_grads()
        self._language_feature = nn.Parameter(torch.zeros((self.points.shape[0], lang_ft_dim), device=self.nerfmodel.device), requires_grad=True).to(self.nerfmodel.device)
        self.lang_ft_dim = lang_ft_dim

    def sample_gaussians(self, cosine_sim, language_thresh, gauss_per_triangle_thresh):
        verts = self.surface_mesh.verts_list()[0]
        faces = self.surface_mesh.faces_list()[0]
        faces_verts = verts[faces]
        print("=============================================================")
        print("Initial vertices              : ", verts.shape)
        print("Initial faces                 : ", faces.shape)

        cosine_sim_mask = (self.sim_vec > language_thresh) * 1.0
        cosine_sim_mask = (cosine_sim_mask.reshape(-1, self.n_gaussians_per_surface_triangle)).sum(dim=-1)
        cosine_sim_mask = cosine_sim_mask > gauss_per_triangle_thresh

        faces_new = faces[cosine_sim_mask].reshape((-1,))
        faces_verts_new = faces_verts[cosine_sim_mask].reshape((-1, 3))

        surface_normals = self.surface_mesh.faces_normals_list()[0].detach().clone()
        surface_normals_new = surface_normals[cosine_sim_mask]

        verts_new = torch.zeros_like(verts)
        verts_new.scatter_(0, faces_new.unsqueeze(-1).expand_as(faces_verts_new), faces_verts_new)
        filter_verts_mask = ~(verts_new == 0).all(dim=1)
        filtered_verts = verts_new[filter_verts_mask]
        filter_verts_mask_ = copy.deepcopy(filter_verts_mask)

        non_zero_indices = torch.nonzero(filter_verts_mask).flatten()
        index_mapping = {old_idx.item(): new_idx for new_idx, old_idx in enumerate(non_zero_indices)}
        updated_faces = torch.tensor([index_mapping[idx.item()] for idx in faces_new])
        updated_faces = updated_faces.reshape((-1, 3))

        print("language conditioned vertices : ", filtered_verts.shape)
        print("language conditioned faces    : ", updated_faces.shape)
        print(max(updated_faces.reshape(-1, )))
        print("=============================================================")

        vertex_colors = torch.ones_like(filtered_verts) * 0.5

        surface_mesh = Meshes(
            verts=[filtered_verts.to(self.device)],
            faces=[updated_faces.to(self.device)],
            textures=TexturesVertex(verts_features=vertex_colors[None].clamp(0, 1).to(self.device)),
            # verts_normals=[verts_normals.to(rc.device)],
        )

        new_o3d_mesh = o3d.geometry.TriangleMesh()
        new_o3d_mesh.vertices = o3d.utility.Vector3dVector(filtered_verts.cpu().numpy())
        new_o3d_mesh.triangles = o3d.utility.Vector3iVector(updated_faces.cpu().numpy())
        new_o3d_mesh.vertex_normals = o3d.utility.Vector3dVector(surface_normals_new.cpu().numpy())
        new_o3d_mesh.vertex_colors = o3d.utility.Vector3dVector(vertex_colors.cpu().numpy())

        param_dict = self.update_lang_gaussians(cosine_sim_mask, filter_verts_mask_)
        param_dict["vert"] = filtered_verts
        param_dict["faces"] = updated_faces

        return surface_mesh, new_o3d_mesh, param_dict
    ...
    @property
    def sh_coordinates_dc(self):
        return self._sh_coordinates_dc

    @property
    def sh_coordinates_rest(self):
        return self._sh_coordinates_rest
    ...
    def get_cos_sim(self, tensor_a, tensor_b):
    ...
    def coarse_lang_mesh(self, text_ft_vec, language_ft, language_thresh, gauss_per_triangle_thresh):
        cosine_sim = self.get_cos_sim(language_ft, text_ft_vec) # [self.language_feature[0], 1]
        cosine_sim_mask = cosine_sim > language_thresh
        self._points = torch.nn.Parameter(self._points[cosine_sim_mask])
        self.all_densities = torch.nn.Parameter(self.all_densities[cosine_sim_mask])
        self._scales = torch.nn.Parameter(self._scales[cosine_sim_mask])
        self._quaternions = torch.nn.Parameter(self._quaternions[cosine_sim_mask])
        self._sh_coordinates_dc = torch.nn.Parameter(self._sh_coordinates_dc[cosine_sim_mask])
        self._sh_coordinates_rest = torch.nn.Parameter(self._sh_coordinates_rest[cosine_sim_mask])
        self._language_feature = torch.nn.Parameter(self._language_feature[cosine_sim_mask])

    def move_gaussians(self, new_centers, new_quats):
        self._points = torch.nn.Parameter(torch.Tensor(new_centers), requires_grad=True)
        self._quaternions = torch.nn.Parameter(torch.Tensor(new_quats), requires_grad=True)

    def update_all_gaussians(self, gauss_dict):
        self._points = torch.nn.Parameter(gauss_dict["points"], requires_grad=False)
        self._quaternions = torch.nn.Parameter(gauss_dict["quaternions"], requires_grad=False)
        self._sh_coordinates_dc = torch.nn.Parameter(gauss_dict["sh_coordinates_dc"], requires_grad=False)
        self._sh_coordinates_rest = torch.nn.Parameter(gauss_dict["sh_coordinates_rest"], requires_grad=False)
        self._scales = torch.nn.Parameter(gauss_dict["scales"], requires_grad=False)
        self.all_densities = torch.nn.Parameter(gauss_dict["densities"], requires_grad=False)
        self._language_feature = torch.nn.Parameter(gauss_dict["language_features"], requires_grad=False)

    def get_gaussians_scene(self):
        with torch.no_grad():
            points = self._points.clone().detach()
            sh_coords_dc = self._sh_coordinates_dc.clone().detach()
            sh_coords_rest = self._sh_coordinates_rest.clone().detach()
            scales = self._scales.clone().detach()
            quaternions = self._quaternions.clone().detach()
            all_densities = self.all_densities.clone().detach()
            ...
            if self.include_feature:
                lang_fts = self.language_feature.clone().detach()
                gaussian_info["language_features"] = lang_fts.to(self.nerfmodel.device)

            return gaussian_info

    def update_gaussians(self, gauss_dict):
        points = self.points.detach()
        sh_coords_dc = self.sh_coordinates_dc.detach()
        sh_coords_rest = self.sh_coordinates_rest.detach()
        scales = self.scaling.detach()
        quaternions = self.quaternions.detach()
        all_densities = self.densities_all.detach()
        lang_fts = self.language_feature.detach()
        ...
        ret_dict = {}
        with torch.no_grad():
            ret_dict["points"] = torch.nn.Parameter(torch.vstack((gaussian_info["points"], gauss_dict["points"])), requires_grad=False)
            ret_dict["all_densities"] = torch.nn.Parameter(torch.vstack((gaussian_info["densities"], gauss_dict["densities"])), requires_grad=False)
            ret_dict["sh_coordinates_dc"] = torch.nn.Parameter(torch.vstack((gaussian_info["sh_coordinates_dc"], gauss_dict["sh_coordinates_dc"])), requires_grad=False)
            ret_dict["sh_coordinates_rest"] = torch.nn.Parameter(torch.vstack((gaussian_info["sh_coordinates_rest"], gauss_dict["sh_coordinates_rest"])), requires_grad=False)
            ret_dict["scales"] = torch.nn.Parameter(torch.vstack((gaussian_info["scales"], gauss_dict["scales"])), requires_grad=False)
            ret_dict["quaternions"] = torch.nn.Parameter(torch.vstack((gaussian_info["quaternions"], gauss_dict["quaternions"])), requires_grad=False)
            ret_dict["language_feature"] = torch.nn.Parameter(torch.vstack((gaussian_info["language_features"], gauss_dict["language_features"])), requires_grad=False)

        return ret_dict

    def language_mesh(self, text_ft_vec, language_ft, language_thresh, gauss_per_triangle_thresh, cosine_sim_mask = None):
        verts = self.surface_mesh.verts_list()[0]
        faces = self.surface_mesh.faces_list()[0]
        faces_verts = verts[faces] 
        print("=============================================================")
        ...
        if cosine_sim_mask is None:
            cosine_sim = self.get_cos_sim(language_ft, text_ft_vec) # [self.language_feature[0], 1]
            cosine_sim_mask = (cosine_sim > language_thresh)*1.0
        ...
        # self._surface_mesh_faces = nn.Parameter(updated_faces, requires_grad=False)
        # face_pts = filtered_verts[updated_faces]
        # points = face_pts[:, None] * self.surface_triangle_bary_coords[None]
        # points = points.sum(dim=-2)
        # param_dict["points"] = nn.Parameter(points.reshape(-1, 3), requires_grad=False)

        return surface_mesh, new_o3d_mesh, param_dict
    
    def update_lang_gaussians(self, filter_meshfaces_mask, filter_points_mask):
        """
        Update the following parameters for language conditioned gaussians
        self._points
        ...
        self._scales
        self._quaternions
        self._sh_coordinates_dc
        self._sh_coordinates_rest
        self._language_feature
        """

        all_densities = torch.nn.Parameter(((self.all_densities.reshape((-1, self.n_gaussians_per_surface_triangle, 1)))[filter_meshfaces_mask]).reshape((-1, 1)))
        _scales = torch.nn.Parameter(((self._scales.reshape((-1, self.n_gaussians_per_surface_triangle, 2)))[filter_meshfaces_mask]).reshape((-1, 2)), requires_grad=False)
        _quaternions = torch.nn.Parameter(((self._quaternions.reshape((-1, self.n_gaussians_per_surface_triangle, 2)))[filter_meshfaces_mask]).reshape((-1, 2)), requires_grad=False)
        _sh_coordinates_dc = torch.nn.Parameter(((self._sh_coordinates_dc.reshape((-1, self.n_gaussians_per_surface_triangle, 1, 3)))[filter_meshfaces_mask]).reshape((-1, 1, 3)), requires_grad=False)
        _sh_coordinates_rest = torch.nn.Parameter(((self._sh_coordinates_rest.reshape((-1, self.n_gaussians_per_surface_triangle, 15, 3)))[filter_meshfaces_mask]).reshape((-1, 15, 3)), requires_grad=False)
        _language_feature = torch.nn.Parameter(((self._language_feature.reshape((-1, self.n_gaussians_per_surface_triangle, self.lang_ft_dim)))[filter_meshfaces_mask]).reshape((-1, self.lang_ft_dim)), requires_grad=False)
        _n_points = self._scales.shape[0]
        ...
        param_dict = {
            ...
            "sh_dc" : _sh_coordinates_dc,
            "sh_rest" : _sh_coordinates_rest,
            "lang_ft" : _language_feature,
            "n_points" : _n_points
        }
        
        return param_dict

    def update_coarse_lang_gaussians(self, prune_mask):
        """
        Update the following parameters for language conditioned gaussians
        self._points
        self._surface_mesh_faces
        ...
        self._language_feature = torch.nn.Parameter(self._language_feature[prune_mask].detach(), requires_grad=False)
    ...
    def render_image_gaussian_rasterizer(
        ...
        if self.include_feature:
            from lang_diff_gaussian_rasterization import GaussianRasterizationSettings, GaussianRasterizer
        else:
        ...
        if self.include_feature:
            raster_settings = GaussianRasterizationSettings(
                ...
                prefiltered=False,
                debug=False,
                include_feature=self.include_feature,
        ...
        # language features
        if self.include_feature:
            language_feature_precomp = self._language_feature
        ...
        # language_feature_precomp = torch.sigmoid(language_feature_precomp)
            ...
            language_feature_precomp = torch.zeros((1), device="cuda:0", requires_grad=False)
            #language_feature_precomp = splat_colors*0.0
        ...
        if not(return_2d_radii or return_opacities or return_colors):
            if not self.include_feature:
                ...
                return rendered_image.transpose(0, 1).transpose(1, 2), language_feature_image
            ...
            if self.include_feature:
                outputs["language_feature_image"] = language_feature_image
            if return_opacities:
    ...
    def render_image_for_AI2THOR(
        ...
        if bg_color is None:
            bg_color = torch.Tensor([0, 0, 0]).to(self.device)
        ...
        p3d_camera = nerf_cameras.p3d_cameras[0]
        use_torch = False

        c2w = c2w.cpu().numpy()  # .transpose(-1, -2)
        # change from OpenGL/Blender camera axes (Y up, Z back) to COLMAP (Y down, Z forward)
        c2w[:3, 1:3] *= -1
        ...
        if point_colors is not None:
            ...
            T_tensor = T_tensor.unsqueeze(0).to(self.device)
            R_tensor = torch.tensor(R)
            R_tensor = R_tensor.unsqueeze(0).to(self.device)
            T_ = Translate(T_tensor, device=T_tensor.device)
            R_ = Rotate(R_tensor, device=R_tensor.device)
            R_ = R_.compose(T_)
            point_depth = R_.transform_points(point_colors)[..., 2:].expand(-1, 3)
            point_colors = point_depth
            max_depth = point_depth.max()
            bg_color = max_depth + torch.zeros(3, dtype=torch.float, device=self.device)
            ...
            if not compute_color_in_rasterizer:
                if sh_rotations is None:
                    ...
                    splat_colors = self.get_points_rgb(
                        positions=positions,
                        camera_centers=None,
                        directions=(torch.nn.functional.normalize(positions - camera_center, dim=-1).unsqueeze(
                            1) @ sh_rotations)[..., 0, :],
                        sh_levels=sh_deg + 1, )
                shs = None
            else:
        ...
        # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means
        # screenspace_points = torch.zeros_like(self._points, dtype=self._points.dtype, requires_grad=True, device=self.device) + 0
        screenspace_points = torch.zeros(self.n_points, 3, dtype=self._points.dtype, requires_grad=True,
                                         device=self.device)
        if return_2d_radii:
            try:
                screenspace_points.retain_grad()
