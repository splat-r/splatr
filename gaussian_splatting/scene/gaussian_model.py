...
class GaussianModel:
    ...
    def capture(self, include_feature = False):
        if include_feature:
            assert self._language_feature is not None
            return (
    ...
    def get_gaussians(self):
        gaussian_dict = {
            "active_sh_deg" : self.active_sh_degree,
            "points" : self._xyz.detach(),
            "sh_coordinates_dc" : self._features_dc.detach(),
            "sh_coordinates_rest" : self._features_rest.detach(),
            "scales" : self._scaling.detach(),
            "quaternions" : self._rotation.detach(),
            "densities" : self._opacity.detach(),
    ...
    def restore(self, model_args, training_args):
        if len(model_args) == 13:
            ...
            self.training_setup(training_args, True)
            ...
            opt_dict, 
            self.spatial_lr_scale) = model_args
            self.training_setup(training_args, False)
    ...
    def training_setup(self, training_args, include_feature = False):
        ...
        if include_feature:
            if self._language_feature is None or self._language_feature.shape[0] != self._xyz.shape[0]:
                language_feature = torch.zeros((self._xyz.shape[0], 8), device="cuda")
