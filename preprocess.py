...
class PrepareDataset:
    def __init__(self, dataset_path, sam_ckpt_path, resolution_=-1):
        seed_num = 42
        self.seed_everything(seed_num)
        ...
        for data_path in data_list:
            ...
            if resolution_ == -1:
                ...
                global_down = orig_w / resolution_
        ...
        self.create(imgs, data_list, save_folder)

    def create(self, image_list, data_list, save_folder):
        ...
        for i, img in tqdm(enumerate(image_list), desc="Embedding images", leave=False):
            timer += 1
            try:
                img_embed, seg_map = self._embed_clip_sam_tiles(img.unsqueeze(0), self.sam_encoder)
            except:
        ...
        for i in range(img_embeds.shape[0]):
            ...
            self.sava_numpy(save_path, curr)

    def sava_numpy(self, save_path, data):
    ...
    def sam_encoder(self, image):
        ...
        if len(masks_s) == 0:
            masks_s = copy.deepcopy(masks_default)
        if len(masks_m) == 0:
            masks_m = copy.deepcopy(masks_default)
        if len(masks_l) == 0:
            masks_l = copy.deepcopy(masks_default)
    ...
    def seed_everything(self, seed_value):
